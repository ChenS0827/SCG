...
def setup_logger(log_file):
    logger = logging.getLogger('deepseekcoder_rag_logger')
...
def calculate_bleu(reference, candidate):
    ...
    """
    try:
        # Do not use lower(); keep original casing
        reference_tokens = word_tokenize(reference)
        candidate_tokens = word_tokenize(candidate)

        # Check if the generated code/tokens are empty
        if not candidate_tokens:
            return 0.0

        # Use method5 smoothing function
        ...
        for line in lines:
            if line not in truncated_lines:
                ...
                if current_token_count + line_tokens <= max_tokens:
                    ...
                    # Attempt partial line (word splitting) if space allows
...
def process_retrieved_contexts(retrieved_contexts, top_k, tokenizer):
    ...
    for i, ctx in enumerate(retrieved_contexts[:top_k]):
        # Construct temp example to check length
        ...
        if token_count >= 2000:
            ...
            # Recalculate for stats (optional)
...
def build_prompt_with_rag(retrieved_contexts, question, tokenizer, top_k=None):
    ...
    Returns a raw string for generation.
    """
...
def generate_code_batch(model, tokenizer, prompts, batch_size=4, max_length=10000, temperature=0.2, top_p=0.9, logger=None):
    """
    Generate code in batches using raw string prompts (not chat template).
    """
    generated_codes = []
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        try:
            ...
            # Fill with empty strings on error to maintain alignment
...
def main():
    parser = argparse.ArgumentParser(description="deepseekcoder-rag")
    parser.add_argument("--model_path", type=str, required=True, help="Path to the model")
    parser.add_argument("--data_path", type=str, required=True, help="Path to retrieval results json")
    parser.add_argument("--output_dir", type=str, default="deepseekcoder_rag_results", help="Directory to save results")
