...
def setup_logger(log_file):
    logger = logging.getLogger('codellama_icl_logger')
...
def extract_solidity_code(text):
    ...
    if matches:
        return matches[-1].strip() # Return the last block if multiple exist
...
def build_icl_prompt(icl_examples, description):
    """
    Build prompt based on the provided PDF template.
    """
    # Header Instruction
    prompt = "### Instruction:\n"
...
def main():
    parser = argparse.ArgumentParser(description="codellama-zero-shot-icl")
    ...
    log_file = os.path.join(args.output_dir, "codellama_icl_log.log")
    ...
    output_file = os.path.join(args.output_dir, "codellama_icl_result.json")
