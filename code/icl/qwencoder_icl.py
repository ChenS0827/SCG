...
def setup_logger(log_file):
    logger = logging.getLogger('qwen_icl_logger')
...
def generate_code_batch(model, tokenizer, prompts, batch_size=1, max_length=8000, temperature=0.2, top_p=0.9, logger=None):
    generated_codes = []
...
def main():
    parser = argparse.ArgumentParser(description="qwencoder-zero-shot-icl")
    ...
    parser.add_argument("--output_dir", type=str, default="qwen_contract_results", help="Directory to save results")
    ...
    log_file = os.path.join(args.output_dir, "qwen_icl_log.log")
    ...
    output_file = os.path.join(args.output_dir, "qwen_icl_result.json")
