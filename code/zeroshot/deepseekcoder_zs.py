...
def setup_logging(log_file="deepseek_generation.log"):
    """
...
def calculate_bleu(reference, candidate):
    ...
    """
    try:
        # Do not use lower(), keep original case
...
def load_model(model_path):
    ...
    try:
        ...
        # Load model with full parameters (float16)
    ...
    try:
        ...
        for sample in data:
            ...
            if all(k in sample for k in ["index", "description"]):
                # 'code' might be named 'reference_code' or 'code' depending on dataset version
...
def generate_code(tokenizer, model, description, max_new_tokens=2048):
    """
    Generate code using the zero-shot prompt template (Fig 7).
    """
    logger = logging.getLogger(__name__)
...
def main():
    ...
    parser.add_argument("--log_file", type=str, default="deepseek_generation.log", 
