...
def preprocess_function(examples, tokenizer):
    ...
    for desc, code in zip(examples["description"], examples["code"]):
        prompt = f"<s>[INST] Generate Solidity code that satisfies: {desc} [/INST] {code}</s>"
    ...
    response_end_token = tokenizer.encode("[/INST]", add_special_tokens=False)
    response_start_len = len(response_end_token)
    
    for i in range(len(labels)):
        input_ids = encodings["input_ids"][i].tolist()
        start_idx = None
        for j in range(len(input_ids) - response_start_len + 1):
            if input_ids[j:j + response_start_len] == response_end_token:
                start_idx = j + response_start_len
                break
