...
def preprocess_function(examples, tokenizer):
    ...
    response_token_id = tokenizer.encode("\n### Response:\n", add_special_tokens=False)
    
    for i in range(len(labels)):
        input_ids = encodings["input_ids"][i].tolist()
        start_idx = None
        for j in range(len(input_ids) - len(response_token_id) + 1):
            if input_ids[j:j+len(response_token_id)] == response_token_id:
                start_idx = j + len(response_token_id)
                break
...
def main():
    parser = argparse.ArgumentParser(description="deepseek-sft")
